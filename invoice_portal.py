...
def _extract_step_summary(action_id, text):
    ...
    if action_id == "invoke-extractor":
        ...
        match_result = re.search(r'(match(?:ed|es)?|mismatch|discrepanc)', text, re.I)
        ...
        # Generic summary - take first meaningful line
    ...
    # Include full raw text in details
    if text:
...
ENDPOINT = os.environ.get(
    ...
    "https://psc-multiagentdemo-001.services.ai.azure.com/api/projects/psc-demo-useast2",
)
        ...
        with client:
            ...
            if uploaded_paths:
                try:
                    ...
                    pass  # continue with doc_text only
            ...
            def stream_and_emit(stream, step_name):
                ...
                for event in stream:
                    if event.type == ResponseStreamEventType.RESPONSE_OUTPUT_TEXT_DELTA:
                        ...
                        emit(session_id, {"type": "step", "step": "error", "status": "error", "message": str(err), "timestamp": _utc_ts()})
                        with sessions_lock:
                            if session_id in sessions:
                                sessions[session_id]["status"] = "error"
                        return False
                
                # Stream finished - check if we should wait for approval
                full = "".join(all_text)
                waiting = saw_approval_question or last_completed_action_id == "invoke-risk-scorer" or ("APPROVE" in full.upper() and "REJECT" in full.upper())
                print(f"[STREAM] Finished, waiting={waiting}, saw_approval={saw_approval_question}, last_action={last_completed_action_id}")
                return waiting

            # 1) Start workflow (answers "get documents" by leaving it for user; we'll send docs next)
            ...
            # 2) Send combined documents (triggers extractor -> matcher -> sql -> risk -> approval question)
            ...
            with sessions_lock:
                if session_id not in sessions:
                    return
                sessions[session_id]["stream_complete"] = True  # Mark stream as fully processed
            ...
            # IMPORTANT: emit and cleanup OUTSIDE the lock to avoid deadlock
            if waiting:
                emit(session_id, {
                    "type": "step",
                    "step": "approval",
...
def api_upload():
    ...
    with sessions_lock:
        sessions[session_id] = {
            ...
            "file_ids": [],
            "stream_complete": False,  # True when workflow stream has fully finished
        }
    emit(session_id, {
        "type": "step",
        "step": "upload",
    ...
    def generate():
        ...
        while True:
            ...
            while idx < len(evs):
                ...
                # Include the data field with raw_output
                if ev.get("data"):
                ...
                # Don't close SSE immediately on waiting - let pending completions come through

            if status == "complete" or status == "error":
                yield f"data: {json.dumps({'status': status})}\n\n"
                return
            
            # Re-read BOTH status and stream_complete together to avoid race condition
            with sessions_lock:
            ...
            if current_status == "waiting" and stream_complete:
                # Stream has fully finished, safe to close
...
def api_approve(session_id):
    ...
    if decision not in ("APPROVE", "REJECT"):
        return Response(json.dumps({"error": "decision must be APPROVE or REJECT"}), status=400, mimetype="application/json")
    with sessions_lock:
        if session_id not in sessions:
        ...
        if sessions[session_id]["status"] != "waiting":
            return Response(json.dumps({"error": "Session not waiting for approval"}), status=400, mimetype="application/json")
